...
public:
	...
	MyClass(T_coll* coll, T_value value) :collection(coll), point(value) { size = 5; }
	~MyClass()
	{
...
template<class T_coll, class T_value>
inline void MyClass<T_coll, T_value>::setCollection(T_coll* coll)
{
	collection = coll;
}
...
int main()
{
	...
	p1.Print();
	NewPoint<double, float> p2(1.222222222222, 5.45);
	p2.Print();
	...
	//const int size = 10;
	//long numbers[size] = { 1,5,8,6,12,14,74,52,36,65 };
	//cout << "Max element : " << GetMax(numbers, size)<<endl;

	//int numbers1[size] = { 1,5,8,6,12,14,74,52,36,65 };
	//cout << "Max element : " << GetMax(numbers1, size) << endl;

	//int numbers2[size] = { 1,5,8,6,12,14,74,52,36,65 };
	...
	////string names[5] = { "Andriy", "Mukola", "Oleksander", "Yura", "Vlad" };
	//string names[5] = { "A", "AA", "AAA", "AAAA", "AAAAA" };
